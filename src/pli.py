...
# Embedded file name: /usr/local/e2/lib/enigma2/python/Plugins/Extensions/FeedsFinder/pli.py
# Compiled at: 2020-10-16 08:40:23
...
class Satfinder(ScanSetup, ServiceScan):
    ...
    def createSetup(self):
        self.list = []
        self.satfinderTunerEntry = getConfigListEntry(_('Tuner'), config.plugins.FeedsFinder.nimnum)
...
def SatfinderMain(session, close=None, **kwargs):
    ...
    nimList = []
    for n in nims:
        if not any([n.isCompatible(x) for x in 'DVB-S']):
            continue
...
def Plugins(**kwargs):
    if any([nimmanager.hasNimType(x) for x in 'DVB-S']):
