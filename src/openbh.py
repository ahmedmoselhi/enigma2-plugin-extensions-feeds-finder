...
# Embedded file name: /usr/lib/enigma2/python/Plugins/Extensions/FeedsFinder/openbh.py
# Compiled at: 2021-05-13 10:26:31
...
class Satfinder(ScanSetup, ServiceScan):
    ...
    def createSetup(self):
        ...
        if self.DVB_type.value == 'DVB-S':
            ...
            if len(nimmanager.getTransponders(int(self.tuning_sat.value), self.feid)) < 1:
            ...
            self.typeOfTuningEntry = getConfigListEntry(_('Tune'), self.tuning_type)
            if config.Nims[self.feid].cable.scan_type.value != 'provider' or len(nimmanager.getTranspondersCable(int(self.satfinder_scan_nims.value))) < 1:
                self.tuning_type.value = 'single_transponder'
            else:
                self.list.append(self.typeOfTuningEntry)
            if self.tuning_type.value == 'single_transponder':
            ...
            self.typeOfTuningEntry = getConfigListEntry(_('Tune'), self.tuning_type)
            if len(nimmanager.getTranspondersATSC(int(self.satfinder_scan_nims.value))) < 1:
                self.tuning_type.value = 'single_transponder'
            else:
                self.list.append(self.typeOfTuningEntry)
            if self.tuning_type.value == 'single_transponder':
    ...
    def createConfig(self, foo):
        self.tuning_type = ConfigSelection(default='single_transponder', choices=[('single_transponder', _('FeedsFinder transponder'))])
    ...
    def keyGoScan(self):
        ...
        if self.DVB_type.value == 'DVB-S':
            ...
            self.addATSCTransponder(tlist, self.transponder[0], self.transponder[1], self.transponder[2], self.transponder[3])
...
class SatfinderExtra(Satfinder):
    ...
    def __init__(self, session):
        ...
        self['actions2'] = ActionMap(['ColorActions'], {'yellow': (self.keyReadServices)}, -3)
    ...
    def getOrbPosFromNit(self, currentProcess):
        ...
        if fd < 0:
            ...
            if not nit_current_content:
                ...
                return
            transponders = [t for t in nit_current_content if 'descriptor_tag' in t and t['descriptor_tag'] == 67 and t['original_network_id'] == self.onid and t['transport_stream_id'] == self.tsid]
            transponders2 = [t for t in nit_current_content if 'descriptor_tag' in t and t['descriptor_tag'] == 67 and t['transport_stream_id'] == self.tsid]
